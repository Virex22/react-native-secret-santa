...
const CreateGroupScreen = ({navigation}) => {
  ...
  function onSubmit() {
    if (forms.nameValid && forms.maxAmountValid) {
      PostGroup(
        forms.name,
        [{name: userContext.profile.pseudo, id: userContext.profile.id}],
        forms.maxAmount,
      ...
      <ScrollView style={styles.container}>
        ...
        <TextInput
          ...
          onChange={(text, isValid) =>
            setForms({...forms, name: text, nameValid: isValid})
          }
        />
        <NumberInput
          onChange={(text, isValid) =>
            setForms({...forms, maxAmount: text, maxAmountValid: isValid})
          }
          placeholder="15"
